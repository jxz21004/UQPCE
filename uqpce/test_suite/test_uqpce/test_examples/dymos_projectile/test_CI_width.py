...
class TestCost(unittest.TestCase):
    def setUp(self):
        ...
        prob.setup()
        prob.set_val('ci_lower', 132.58208526)
        prob.set_val('ci_upper', 167.91431536)
    ...
    def test_partials(self):
        partial_lb = self.partials['dist'][('width', 'ci_lower')]['rel error'][0]
        partial_ub = self.partials['dist'][('width', 'ci_upper')]['rel error'][0]
    ...
    def test_compute(self):
        ub = self.prob.get_val('ci_upper')
        lb = self.prob.get_val('ci_lower')
        calc = ub - lb
